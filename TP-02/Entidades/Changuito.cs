...
namespace Entidades_2018
{
    ...
    public sealed class Changuito
    {
        ...
        {
            bool estaElProducto = false;
            foreach(Producto otroProducto in c.productos)
            {
                if(otroProducto == p)
                {
                    estaElProducto = true;
        ...
        public static Changuito operator -(Changuito c, Producto p)
        {
            foreach (Producto productoListado in c.productos)
            {
                if (productoListado == p)
                {
                    c.productos.Remove(productoListado);
                    break;
                }
            }
            return c;
